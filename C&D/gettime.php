...
$num = $HTTP_POST_VARS ["number"];
$date = $HTTP_POST_VARS ["date"];
$doc = $HTTP_POST_VARS ["doc"];
$interval = $HTTP_POST_VARS ["interval"];
$id = $_SESSION['userdetail'];
if ($interval < 7 || $interval > 22)
{
$ermsg = "choose a correct time between 7 - 22 o'clock";
$_SESSION['errortime'] = $ermsg;
...
$ermsg = "choose a correct Date, the mentioned date pasts";
$_SESSION['errordate'] = $ermsg;
header ("Location: bank.php");
} 

$sql = mysql_query("SELECT * FROM doctime WHERE date = '$date' AND docid = '$doc' AND time = '$interval'");
$row = mysql_fetch_array($sql);
if (!$row)
{
mysql_query("INSERT INTO doctime VALUES ('$date', '$interval', '$id', '$num', '$doc' )");
$address = mysql_query("SELECT * FROM user WHERE ID = '$doc'");
while ($row = mysql_fetch_array($address))
{
$adrs = $row['address'];
}
$msg = "you will visited by doctor in $adrs in $date , $interval o'clock";
$_SESSION['doccheckedmsg'] = $msg;
header ("Location: home.php");
}
else
{
$ermsg = "this time is filled by another user take another time";
$_SESSION['errortime'] = $ermsg;
header ("Location: bank.php");
}
mysql_close($con);
